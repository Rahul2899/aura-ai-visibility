...
async def _fetch_safe(start_url: str, name: str) -> str | None:
    """Fetch a homepage from an ALREADY-SSRF-validated start URL, following redirects
    manually so every hop is re-validated. Real sites 301 the apex to www/ or a region
    page, but a redirect target is controlled by the response — auto-follow would let a
    ...
    async with httpx.AsyncClient(timeout=8, follow_redirects=False) as client:
        ...
        for _ in range(6):  # initial request + up to 5 redirects
            ...
            if r.is_redirect and r.headers.get("location"):
                ...
                # private/metadata IPs) regardless of the redirect's scheme.
...
async def _scrape_homepage(name: str, domain: str) -> str | None:
    ...
    Tries HTTPS first. If HTTPS fails at the transport layer (common for small/legacy
    sites with broken TLS — e.g. a German gym whose cert handshake errors), falls back
    to HTTP on the SAME already-validated host. SSRF safety is unchanged: the host's IP
    was vetted by _safe_https_url and every redirect hop is re-validated; the only
    trade-off is that the HTTP fetch reads the page in plaintext."""
    safe_url = _safe_https_url(domain)
    if not safe_url:
        log.warning("homepage_fetch_blocked_ssrf", brand=name, domain=domain)
        return None
    # safe_url is "https://{validated_host}" — reuse that exact host for the HTTP fallback
    # so we never re-open the SSRF gate to an unvalidated string.
    host = safe_url[len("https://"):]
    try:
        result = await _fetch_safe(safe_url, name)
        if result:
            return result
    except Exception as e:
        log.warning("homepage_https_failed_trying_http", brand=name, domain=domain, error=str(e))
        try:
            result = await _fetch_safe(f"http://{host}", name)
            if result:
                return result
